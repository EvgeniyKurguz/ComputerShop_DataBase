...
public class ConnectionPool {
    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(ConnectionPool.class);
        ...
        try {
            if (connectionList.isEmpty()) {
                if (CURRENT_POOL_SIZE >= MAX_POOL_SIZE) {
                    LOGGER.info("Maximum pool size is reaced. Can not create connection");
                }
                CURRENT_POOL_SIZE++;
